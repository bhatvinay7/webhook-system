...
```bash
docker compose exec postgres psql -U stripe -d stripe_demo -c "SELECT pg_create_logical_replication_slot('sequin_slot', 'pgoutput');"
```
...
```bash
docker compose exec postgres psql -U stripe -d stripe_demo -c \
