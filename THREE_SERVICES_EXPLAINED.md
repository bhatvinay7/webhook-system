...
## SERVICE 1: new-api (Port 3001) - Core API Service
...
**ENDPOINTS:**
1. `POST /auth/register`: Create a new merchant account.
2. `POST /auth/login`: Authenticate and get a merchant ID.
3. `POST /keys/generate`: Generate Ed25519 keypairs (returns public key, stores private key securely).
4. `POST /payments`: Create a new payment.

**WHAT HAPPENS (Payment Flow):**
1. Generates payment ID (UUID)
2. Saves to DB: `INSERT INTO payments (id, merchant_id, amount, currency, status) VALUES (...)`
3. PostgreSQL TRIGGER fires automatically → creates event in `domain_events` table
4. **Audit Ledger**: A background Tokio task receives an event via an MPSC channel and inserts it into the `audit_ledger` table without blocking the main HTTP response.
